...
/*        XidStore x = new XidStore();

        if (x.write("xid1", "xid2")) {
            x.read(new ArrayList<>()).forEach(System.out::println);

            System.out.printf("removing xid2");
            x.remove("xid2");
    ...
    public void remove(Xid xid) throws Exception {
        FileInputStream fos= new FileInputStream(xidFileName);
        FileLock fl = fos.getChannel().tryLock(0L, Long.MAX_VALUE, true);
//        String xidStr = xidToString.apply(xid);
...
/*    private Function<Xid, String> xidToString = new Function<Xid, String>() {
        public String apply(Xid xid) {
